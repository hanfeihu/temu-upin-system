      ...
      >
        <a-form layout="vertical" :model="editForm">
          <a-form-item label="过滤词" required>
            <a-input v-model:value="editForm.word" placeholder="例如: 侵权/品牌词/二维码" />
          </a-form-item>
        </a-form>
      </a-modal>
    </div>
...
const reload = async () => {
  loading.value = true
  try {
    const res = await ocrApi.listFilterWords()
    if (res?.success) {
...
const save = async () => {
  ...
  try {
    const res = editForm.id
      ? await ocrApi.updateFilterWord(editForm.id, payload)
      : await ocrApi.createFilterWord(payload)
    if (res?.success) {
      message.success('已保存')
      editOpen.value = false
      await reload()
...
const confirmDelete = (r) => {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const res = await ocrApi.deleteFilterWord(r.id)
        if (res?.success) {
          message.success('已删除')
          await reload()
          return
