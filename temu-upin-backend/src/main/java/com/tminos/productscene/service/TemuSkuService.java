...
public class TemuSkuService {
    ...
    public List<TemuSkuDTO.TemuSkuRow> saveAll(Long spuId, List<TemuSkuDTO.TemuSkuRow> rows) {
        ...
        for (TemuSkuDTO.TemuSkuRow r : rows) {
            ...
            e.setSpecJson(trimOrNull(r.getSpecJson()));
    ...
    public BigDecimal computeSupplyPrice(BigDecimal originSkuPrice, BigDecimal baseFreight, Integer weightG) {
        ...
        // ((货值(1688 sku 价 + 基础运费) + 6) + (重量 * (69/1000)))
        BigDecimal goodsValue = p.add(freight);
        BigDecimal add6 = goodsValue.add(new BigDecimal("6"));
        BigDecimal perG = new BigDecimal("0.069");
        BigDecimal weightCost = perG.multiply(new BigDecimal(String.valueOf(g)));
        // Final *3 (business rule: supply price is 3x)
        return add6.add(weightCost)
     ...
     * - If contains CJK, translate to English (best-effort)
    ...
    private String normalizeSpecKeyForTemuDisplay(String specKey, String fallback) {
        ...
        if (containsCjk(t)) {
            String tr = translateToEnBestEffort(t);
    ...
    /**
     * Reuse the same translate util pattern as TemuPublishService (reflection, no hard dependency).
     */
    private String translateToEnBestEffort(String text) {
