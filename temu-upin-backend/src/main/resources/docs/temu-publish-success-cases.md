...
补充：重复重新生成规格映射草案并不一定会自动修好这类问题。如果草案生成逻辑仍然把已经识别出的多维结果压回成单维，那么重新保存后落库的草案依旧会是：

- `dimensions=尺码`
- `skuPlans=8`
- `skcPlans=1`

也就是 8 条 SKU 继续共用同一组 `尺码=均码` 销售属性，发布时仍然会报重复。真正要修的是 **规格映射草案生成阶段的维度归一化逻辑**，而不是只重复点击“重新生成”。
